...
// Bounded, ephemeral receipt metadata and 180-character reply summaries. No media stored here.
module.exports = function roomFeatures(io) {
  ...
  function record(socket, id, summary = {}) {
    ...
    const r = { id, senderId: socket.id, recipients, created: Date.now(),
      quote: { id, senderId: socket.id, username: socket.username, kind: summary.kind || "text",
        text: String(summary.text || "Message").slice(0, 180) } };
  ...
  return {
    ...
    reply(socket, replyTo) {
      if (replyTo == null) return { value: null };
      if (typeof replyTo !== 'string') return { error: 'Invalid reply target.' };
      const r = rooms.get(socket.room)?.messages.get(replyTo);
      if (!r || Date.now() - r.created > TTL || (r.senderId !== socket.id && !r.recipients.has(socket.id)))
        return { error: 'The original message is no longer available to reply to. Cancel the reply and send again.' };
      return { value: { ...r.quote } };
    },
    joined(socket) {
      socket.emit('room-theme', { ...state(socket.room).theme });
      socket.emit('room-ready', { room: socket.room, selfId: socket.id, username: socket.username, isAdmin: Boolean(socket.isAdmin) });
    }
  };
};
