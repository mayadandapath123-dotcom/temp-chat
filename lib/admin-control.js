...
module.exports = function createAdmin({ app, io, calls, controls, env = process.env, now = Date.now }) {
  ...
  function headers(_req, res, next) {
    res.set({ 'Cache-Control': 'no-store', 'X-Content-Type-Options': 'nosniff', 'Referrer-Policy': 'no-referrer', 'X-Frame-Options': 'DENY',
      'Content-Security-Policy': "default-src 'self'; script-src 'self'; style-src 'self'; img-src 'self' data:; connect-src 'self'; frame-ancestors 'none'; base-uri 'none'; form-action 'self'; object-src 'none'" }); next();
  }
  ...
  for (const file of ['admin.js', 'admin.css']) app.get('/admin-assets/' + file, headers, (_req, res) => res.sendFile(path.join(__dirname, '..', 'admin', file)));
